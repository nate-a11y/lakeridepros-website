...
export function CoordinatorDashboard() {
  ...
  function exportCsv() {
    ...
    const rows = [["Reference", "Rider", "Status", "Ride type", "Ride date", "Pickup", "Destination", "Moovs cost"], ...requests.map((request) => [request.reference, request.riderName, request.status, request.rideTypeName, request.rideDate, request.pickupName, request.destinationName, request.trips.reduce((sum, trip) => sum + (trip.cost ?? 0), 0)])]
