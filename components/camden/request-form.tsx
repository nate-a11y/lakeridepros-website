    ...
    <PortalShell context={data.context}>
      <div className="mx-auto max-w-3xl">
        ...
        <form onSubmit={submit} className="mt-7 space-y-7 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm sm:p-7">
          ...
          <fieldset><legend className="text-lg font-extrabold">Approved locations</legend><p className="mt-2 text-sm text-neutral-600">Your approved home and program locations can be used for pickup or drop-off, including rides back home.</p><div className="mt-3 grid gap-5 sm:grid-cols-2"><div><label htmlFor="pickup-location" className="mb-1 block text-sm font-bold">Pickup <span aria-hidden="true">*</span></label><select id="pickup-location" className={fieldClass} value={draft.pickupLocationId} onChange={(event) => update("pickupLocationId", event.target.value)} required disabled={isCoordinator && !draft.riderId}><option value="">{isCoordinator && !draft.riderId ? "Select a rider first" : "Select approved pickup"}</option>{availablePickups.map((location) => <option key={location.id} value={location.id}>{location.name} — {location.address}</option>)}</select></div><div><label htmlFor="destination" className="mb-1 block text-sm font-bold">Destination <span aria-hidden="true">*</span></label><select id="destination" className={fieldClass} value={draft.destinationLocationId} onChange={(event) => update("destinationLocationId", event.target.value)} required disabled={isCoordinator && !draft.riderId}><option value="">{isCoordinator && !draft.riderId ? "Select a rider first" : "Select approved destination"}</option>{availableDestinations.map((location) => <option key={location.id} value={location.id}>{location.name} — {location.address}</option>)}</select><button type="button" onClick={() => { setLocationFeedback(null); setShowLocationRequest(true) }} aria-haspopup="dialog" className="mt-2 min-h-11 text-sm font-bold text-[#245f0b] underline">Ask for another approved location</button></div></div></fieldset>
