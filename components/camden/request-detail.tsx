...
import { ArrowLeft, CalendarDays, Clock3, Copy, MapPin, MessageSquareText, Pencil, Send, UserRound } from "lucide-react"
...
export function RequestDetailView() {
  ...
  async function submitFollowup(event: FormEvent) {
    ...
    setSaving(true); setFeedback(null)
    try { await createCamdenPortalService(persona).createFollowup(id, action, reason, explanation); setAction(null); setReason(""); setExplanation(""); setFeedback(`${action === "change" ? "Change" : "Cancellation"} request submitted.`); await reload() }
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <section aria-labelledby="itinerary-heading" className="mt-6 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm sm:p-7">
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2">
            ...
            {isCoordinator && <div><p className="text-xs font-bold uppercase text-neutral-600">Assigned LRP operator</p><div className="mt-1 font-semibold">{request.assigneeName ?? "Unassigned"}</div></div>}
            {isCoordinator && <div><p className="text-xs font-bold uppercase text-neutral-600">Moovs cost</p><div className="mt-1 font-extrabold">{syncedCost == null ? "Not available until synced" : syncedCost.toLocaleString("en-US", { style: "currency", currency: "USD" })}</div></div>}
          </div>
