  ...
  return (
    <PortalShell context={data.context}>
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-sm font-bold text-[#245f0b]">Welcome back, {data.context.displayName.split(" ")[0]}</p><h1 className="mt-1 text-3xl font-extrabold tracking-tight sm:text-4xl">Your transportation</h1><p className="mt-2 max-w-2xl text-neutral-600">Request and manage approved Camden County rides.</p></div>
      ...
      <section aria-labelledby="confirmed-heading" className="mt-9">
        <div className="mb-4 flex items-end justify-between gap-4"><div><h2 id="confirmed-heading" className="text-2xl font-extrabold">Confirmed rides</h2><p className="mt-1 text-sm text-neutral-600">Rides linked to a confirmed Moovs reservation.</p></div><Link href="/camden-county/history" className="inline-flex min-h-11 items-center font-bold text-[#245f0b] hover:underline">View history <ArrowRight className="ml-1 size-4" aria-hidden="true" /></Link></div>
        {confirmed.length ? <div className="grid gap-4 xl:grid-cols-2">{confirmed.map((request) => <RequestCard key={request.id} request={request} />)}</div> : <EmptyState title="No confirmed rides" message="A ride appears here once Lake Ride Pros creates and links its reservation." />}
      </section>
    </PortalShell>
  )
}
