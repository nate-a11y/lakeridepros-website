...
export default function EventWaitlistAdminPage() {
  ...
  const loadEntries = useCallback(async () => {
    ...
    setError(null)
    try {
      const json = await callApi({ action: 'list_entries' })
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-gray-600 dark:text-gray-300">
              {entries.length} total request{entries.length === 1 ? '' : 's'} across sold-out event vehicle types.
            </p>
          </div>
        ...
        <section aria-labelledby="waitlist-summary" className="mt-8 rounded-2xl bg-white p-6 shadow dark:bg-dark-bg-secondary">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300">Top event + vehicle combinations.</p>
            </div>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filteredEntries.map((entry) => (
                  <tr key={entry.id} className="align-top text-lrp-black dark:text-white">
                    ...
                    <td className="px-4 py-4 font-semibold">{entry.ride_type_label}</td>
                    <td className="px-4 py-4">
