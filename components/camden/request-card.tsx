  ...
  return (
    <article className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
      ...
      <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
        ...
        <div className="flex gap-2 sm:col-span-2"><MapPin className="mt-0.5 size-4 shrink-0 text-neutral-500" aria-hidden="true" /><div className="min-w-0"><span className="sr-only">Route</span><span className="block break-words"><strong>{request.pickupName}</strong> — {request.pickupAddress || "Address unavailable"}</span><ArrowRight className="my-1 size-4" aria-label="to" /><span className="block break-words"><strong>{request.destinationName}</strong> — {request.destinationAddress || "Address unavailable"}</span></div></div>
      </div>
